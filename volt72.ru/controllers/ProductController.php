			...
			else
			{
				...
				if (isset($category) AND Validate::isLoadedObject($category))
				{
					self::$smarty->assign(array(
						...
						//'path' => (isset($category->id) AND $category->id) ? Tools::getPath(intval($category->id), $this->product->name, false) : Tools::getPath(intval($category->id_category_default), $this->product->name, false),
						'category' => $category,
