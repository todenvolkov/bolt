...
class ContactControllerCore extends FrontController
{
	...
	public function preProcess()
	{
		...
		if (Tools::isSubmit('submitMessage'))
		{
			...
			{
				$extension = array('.txt', '.rtf', '.doc', '.docx', '.pdf', '.zip', '.png', '.jpeg', '.gif', '.jpg', '.rar', '.xls', '.xlsx');
			...
			elseif  ( (int)(Tools::getValue('id_contact')) != 1000 AND (!($id_contact = (int)(Tools::getValue('id_contact'))) OR !(Validate::isLoadedObject($contact = new Contact((int)($id_contact), (int)(self::$cookie->id_lang))))) )
				$this->errors[] = Tools::displayError('Please select a subject on the list.');
			
			elseif  ( (int)(Tools::getValue('id_contact')) == 1000 AND empty($_POST['other']) )
				$this->errors[] = Tools::displayError('Укажите свою тему.');
			...
			elseif (!empty($_FILES['fileUpload']['name']) AND !in_array(substr(strtolower($_FILES['fileUpload']['name']), -4), $extension) AND !in_array(substr($_FILES['fileUpload']['name'], -5), $extension))
			...
			else
			{
				...
				$contact->email = !empty($contact->email) ? $contact->email : Configuration::get('PS_SHOP_EMAIL');
