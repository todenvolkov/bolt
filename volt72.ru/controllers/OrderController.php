...
class OrderControllerCore extends ParentOrderController
{
	...
	{
		global $isVirtualCart, $orderTotal, $cookie;
		...
		$addr = new Address((int)self::$cart->id_address_delivery);
		if($this->step == 2 && Tools::isSubmit('processAddress') && $addr->dni){
			
			include(dirname(__FILE__).'/../modules/bankwire/bankwire.php');
			
			$s2_customer = new Customer((int)self::$cart->id_customer);
			if (!Validate::isLoadedObject($s2_customer))
				Tools::redirectLink(__PS_BASE_URI__.'order.php?step=1');
				
			$s2_currency = new Currency($cookie->id_currency);
			$s2_total = (float)self::$cart->getOrderTotal(true, Cart::BOTH);
			...
			$bankwire->validateOrder(self::$cart->id, 3, $s2_total, $bankwire->displayName, NULL, $mailVars, (int)$s2_currency->id, false, $s2_customer->secure_key);
			$order = new Order($bankwire->currentOrder);
			Tools::redirectLink(__PS_BASE_URI__.'order-confirmation.php?id_cart='.self::$cart->id.'&id_module='.$bankwire->id.'&id_order='.$bankwire->currentOrder.'&key='.$s2_customer->secure_key, false);
		}
	...
	public function autoStep()
	{
		...
		//$phone = new Address((int)(self::$cart->id_phone_invoice));
	...
	public function processAddress()
	{
		...
		{   //echo Tools::getValue('id_phone_delivery'); die();
			self::$cart->id_address_delivery = (int)(Tools::getValue('id_address_delivery'));
			self::$cart->id_phone_delivery = (int)(Tools::getValue('id_phone_delivery'));
			...
			//self::$Address->id_phone_delivery = (int)(Tools::getValue('id_phone_delivery'));
			//self::$Address->update();
