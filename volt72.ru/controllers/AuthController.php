...
class AuthControllerCore extends FrontController
{
	...
	public function preProcess()
	{
        ...
        self::$smarty->assign('captcha', md5(microtime())); 
...
//echo $_SESSION['captcha_keystring'];		
		...
		if (Tools::isSubmit('submitAccount') OR Tools::isSubmit('submitGuestAccount'))
		{
			...
			if ( Tools::getValue('is_new_customer') == 1 )
			{			
			    if (!empty($_POST['passwd']) AND Tools::getValue('passwd') != Tools::getValue('repasswd'))
				    $this->errors[] = Tools::displayError('Пароли должны совпадать');				
            }
			if (!Tools::getValue('phone') AND !Tools::getValue('phone_mobile') AND !Tools::getValue('job_mobile'))
				$this->errors[] = Tools::displayError('You must register at least one phone number');
				
			if (!Tools::getValue('capcha_code') OR $_SESSION['captcha_keystring'] != Tools::getValue('capcha_code'))
				$this->errors[] = Tools::displayError('Код подтверждения не введен или введен неверно');
				
			if (!Validate::isEmail($email = Tools::getValue('email')) OR empty($_POST['email']))
				$this->errors[] = Tools::displayError('Invalid e-mail address');				
			elseif ($this->emailExists($email))
				$this->errors[] = Tools::displayError('An account is already registered with this e-mail, please fill in the password or request a new one.');
	
			if ($_POST['select_type'] == 2 AND empty($_POST['company']))
				    $this->errors[] = Tools::displayError('Заполните поле организация');	
				//echo self::$cookie->captcha_keystring;
						...
						else
						{
							...
							else
							{
								if (!$customer->is_guest)
								{
									...
									array('{customer_id}' => $customer->id, '{confirmation}' => base64_encode($customer->email), '{firstname}' => $customer->firstname, '{lastname}' => $customer->lastname, '{email}' => $customer->email, '{passwd}' => Tools::getValue('passwd')), $customer->email, $customer->firstname.' '.$customer->lastname))
								...
								if ( Tools::getValue('is_new_customer') == 1 ) { self::$cookie->logged = 0; }else{ self::$cookie->logged = 1; } /////////////////
								...
								if (Tools::isSubmit('submitAccount'))
								{
								    Tools::redirect('confirmation.php?c=1');  
								}
	...
	protected function emailExists($email, $return_id = false, $ignoreGuest = true)
	{
