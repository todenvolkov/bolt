<li class="ajax_block_product {if $smarty.foreach.homeFeaturedProducts.first}first_item{elseif $smarty.foreach.homeFeaturedProducts.last}last_item{else}item{/if} {if $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 0}last_item_of_line{elseif $smarty.foreach.homeFeaturedProducts.iteration%$nbItemsPerLine == 1}clear{/if} {if $smarty.foreach.homeFeaturedProducts.iteration > ($smarty.foreach.homeFeaturedProducts.total - ($smarty.foreach.homeFeaturedProducts.total % $nbItemsPerLine))}last_line{/if}">
                                {if $product.on_sale}<h5 class="display"
                                ...
                                <h5 class="display"><a href="{$product.link}"
                                    ...
                                    <div class="display" id="product_reference"
                                         style="text-align: center; margin-top: -23px;">{if $product.reference}<label
                                            for="product_reference">Артикул: {$product.reference}</label>{/if}</div>
                               ...
                               {if $product.on_sale}<h5 class="thumb"
                                ...
                                <div id="feature2">
                                    <p id="product_reference">{if $product.reference}<label for="product_reference">Артикул: {$product.reference}</label>{/if}
                                    </p>
                                </div>
                                {if $product.description_short}
                                    <div id="feature2"
                                         style="height: 90px;">{$product.description_short|truncate:350:'...'|strip_tags}</div>
                                    <div id="feature2"
                                         style="margin-left: 572px; margin-bottom: -16px; z-index: 1000; position: absolute;">
                                ...
                                <div class="priceblock">
                                    {if ($product.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product.available_for_order AND !isset($restricted_country_mode) AND $product.minimal_quantity == 1 AND $product.customizable != 2 AND !$PS_CATALOG_MODE}
                                        ...
                                        {/literal}
                                            ...
                                            {if $product.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}
                                                <p class="price_container"><span
                                                        class="price">{if !$priceDisplay}{convertPrice price=$product.price}{else}{convertPrice price=$product.price_tax_exc}{/if}</span>
                                                </p>{else}
                                        ...
                                        <div style="height:32px;"></div>
                                    {/if}
                                    <div class="add_ok">{l s='Product Added to Cart' mod='homefeatured'}
