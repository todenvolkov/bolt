...
class BlockCategories extends Module
{
	...
	public function getTree($resultParents, $resultIds, $maxDepth, $id_category = 1, $currentDepth = 2)
	{
		global $link;
		...
		if (isset($resultParents[$id_category]) AND sizeof($resultParents[$id_category]) AND ($maxDepth == 0 OR $currentDepth < $maxDepth))
			if($currentDepth==1){
				/**/array_push($children,array(
					"id"=>2, 
					"link"=>$link->getCategoryLink(2, $resultIds[2]['link_rewrite']),
					"name"=>$resultIds[2]['name'],
					"desc"=>NULL,
					"children"=>array()
				));
			}
	...
	public function hookLeftColumn($params)
	{
		...
		if (!$this->isCached('blockcategories.tpl', $smartyCacheId))
		{
			...
			/*array_push($blockCategTree["children"],array(
													"id"=>1, 
													"link"=>$link->getCategoryLink(1, 'home'),
													"name"=>"Обзор каталога",
													"desc"=>NULL,
													"children"=>array()
												));
                //array_pop($blockCategTree["children"]);
			
			array(5) {
      ["id"]=>
      string(1) "2"
      ["link"]=>
      string(26) "http://volt72.ru/2-all"
      ["name"]=>
      string(27) "Обзор каталога"
      ["desc"]=>
      NULL
      ["children"]=>
      array(0) {
		...
		$smarty->cache_lifetime = 1;//31536000; // 1 Year
