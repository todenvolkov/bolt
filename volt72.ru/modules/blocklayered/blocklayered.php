...
class BlockLayered extends Module
{
	...
	public function hookHeader($params)
	{
		...
		//for sorting without layered navigation
		echo '
<script type="text/javascript">
...
<form id="layered_form">
	<input type="hidden" name="id_category_layered" value="'.(int)Tools::getValue('id_category').'" />
</form>
<div id="layered_ajax_loader" style="display: none;">
		<p><img src="/img/loader.gif" alt="" /><br />Загрузка ...</p>
	</div>
';
		...
		foreach ($filters as $filter)
		{
			...
			switch ($filter['type'])
			{
				...
				case 'id_feature':
					...
					AND c.nright <= '.(int)$parent->nright : 'c.id_category = '.(int)$id_parent).'
					AND c.active = 1)) ';
					$sql_query['group'] = 'GROUP BY fv.id_feature_value ORDER BY fvl.value+0, fvl.value';
