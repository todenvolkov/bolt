...
class AdminCategories extends AdminTab
{
    ...
    public function postProcess($token = NULL)
	{
		...
		if (Tools::isSubmit('submitAdd'.$this->table))
		{
			...
			if (!$id_category = (int)(Tools::getValue('id_category'))){
				...
				$this->copyFromPost($category, 'category');
	...
	public function displayForm($token = NULL)
	{
		...
		foreach ($this->_languages AS $language)
			echo '
					<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: left;">
						<textarea class="rte" name="description_'.$language['id_lang'].'" rows="10" cols="100">'.$this->getFieldValue($obj, 'description', (int)($language['id_lang'])).'</textarea>
					</div>';
		// TinyMCE
		global $cookie;
