...
<h1 style="width: 100%; font-size: 26px; font-weight: bold; color: gray; font-family: Calibri; clear: both; text-transform: uppenspace; margin-bottom: 8px;">{l s='Your addresses'}</h1>
...
<form action="{$link->getPageLink('address.php', true)}" method="post" class="std">
	<fieldset>
        ...
        {assign var="is_comp" value=0}
		{if isset($smarty.post.dni)}{$is_comp=1}{else}{if isset($address->dni) && $address->dni|strlen>0}{$is_comp=1}{/if}{/if}
        {literal}
<script>
$(document).ready(function() {
    //$('#j').hide();
...
{/literal}
		 ...
		 <p class="radio required" style="margin-left: 16px;">
                <input type="radio" name="select_type" id="select_type_1" value="1" {if $is_comp == 0}checked="checked"{/if} />
                <label for="select_type_1" class="top">Физическое лицо</label>
                <input type="radio" name="select_type" id="select_type_2" value="2" {if $is_comp == 1}checked="checked"{/if} />
	...
	{/if}
		...
		{if $field_name eq 'company'}
            <p class="required text" id="_company">
                <label for="company">{l s='Company'}</label>
                <input type="text" id="company" name="company" value="{if isset($smarty.post.company)}{$smarty.post.company}{else}{if isset($address->company)}{$address->company|escape:'htmlall':'UTF-8'}{/if}{/if}" /><sup>*</sup>
            </p>
            <p class="required text dni" id="_dni">
        ...
        <input type="hidden" id="phone_mobile" name="phone_mobile" value="{$customer.phone_mobile}" />
        <input type="hidden" id="job_mobile" name="job_mobile" value="{$customer.job_mobile}" />
 
		*}
		<p style="margin-left:5px !important;">{l s='You must register at least one phone number'} <sup style="color:red;">*</sup></p>
