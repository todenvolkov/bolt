...
*}
<h1 style="width: 100%; font-size: 26px; font-weight: bold; color: gray; font-family: Calibri; clear: both; text-transform: uppenspace; margin-bottom: 8px;">{l s='Customer Service'} - {if isset($customerThread) && $customerThread}{l s='Your reply'}{else}{l s='Contact us'}{/if}</h1>
	...
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
			...
			{else}
				...
				{/foreach}
				    <option value="1000" {if isset($smarty.post.id_contact) && $smarty.post.id_contact == $contact.id_contact}selected="selected"{/if}>Другое</option>
				</select>
			</p>
			<p id="desc_contact0" class="desc_contact">&nbsp;</p>
			    {foreach from=$contacts item=contact}					
					<p id="desc_contact{$contact.id_contact|intval}" class="desc_contact" style="display:none; padding-left: 8px;">
						<label>&nbsp;</label>{$contact.description|escape:'htmlall':'UTF-8'}
					</p>
					
				{/foreach}
				    <p id="desc_contact1000" class="desc_contact" style="display:none; padding-left: 160px;">
						<input type="text" id="other" name="other" style="width: 329px;" value="{$smarty.post.other}" placeholder="Напишите свою тему"/>
					</p>
			{/if}
			<p class="text">
		...
		{if $fileupload == 1}
			...
			<p style="padding: 0px 130px 10px 170px;">*.txt, *.rtf, *.doc, *.docx, *.xls, *.xlsx, *.pdf, *.zip, *.rar, *.png, *.jpeg, *.gif, *.jpg</p>
		{/if}
		<p class="textarea">
