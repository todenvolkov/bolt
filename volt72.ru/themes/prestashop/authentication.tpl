...
{/if}
<style>#center_column p { margin: 0.5em 0; padding-left: 0.7em; padding-top: 0.9em; }</style>
<h1 style="width: 100%; font-size: 26px; font-weight: bold; color: gray; font-family: Calibri; clear: both; text-transform: none; margin-bottom: 11px;">АВТОРИЗАЦИЯ\РЕГИСТРАЦИЯ</h1>

{capture name=path}{l s='Login'}{/capture}
{include file="$tpl_dir./breadcrumb.tpl"}
<div id="cart" {if isset($errors) && $errors}style="min-height: 368px;"{else}style="min-height: 300px;"{/if}>
	...
	<form action="{$link->getPageLink('authentication.php', true)}" method="post" id="create-account_form" class="std">
		<fieldset>
			<h3>{l s='Create your account'}</h3>
						<p style="padding-left: 17px; color: #0071b7; font-size: 14px; margin-top: -14px;">При регистрации постоянным клиентам предоставляются дополнительные скидки!</p>

			<h4 style="height: 1em; padding:0 0em 1em 1.5em;">{l s='Enter your e-mail address to create an account'}.</h4>
			<p class="text">
		...
		<form action="{$link->getPageLink('authentication.php', true)}?back={$back}" method="post" id="new_account_form" class="std">
			<fieldset>
				...
				<div id="opc_account_form" style="display: block; ">
					<!-- Account -->
                    <p class="radio required" style="margin-left: 16px; margin-bottom: 16px;">

                                 <input type="radio" name="select_type" id="select_type_1" value="1" {if $smarty.post.select_type == 1}checked="checked"{/if} {if isset($smarty.post.select_type) == false}checked="checked"{/if} />
                                 <label for="select_type_1" class="top">Физическое лицо</label>
                                 <input type="radio" name="select_type" id="select_type_2" value="2" {if $smarty.post.select_type == 2}checked="checked"{/if} />
                                 <label for="select_type_2" class="top">Юридическое лицо</label>
                             </p>
					...
					<p class="radio required" style="display:none;">
						<span>{l s='Title'}</span>
						<input type="radio" name="id_gender" id="id_gender1" value="1" checked="checked" {if isset($smarty.post.id_gender) && $smarty.post.id_gender == '1'}checked="checked"{/if}>
					...
					<p class="required text">
						...
						<sup id="req" style="display:none">*</sup>
					</p>
					<p class="select" style="display:none;">
						<input type="hidden" id="days" name="days" value="1"/>
						<input type="hidden" id="months" name="months" value="1"/>
						<input type="hidden" id="years" name="years" value="1970"/>
...
{/literal}	
					...
					{foreach from=$dlv_all_fields item=field_name}
						...
						<p class="required text" id="_company"  {if $smarty.post.select_type == 1 || !isset($smarty.post.select_type)}style="display: none;"{/if}>
							<label for="company" >{l s='Company'}</label>
							<input type="text" class="text" id="company" name="company" placeholder='Например ООО "Акватерм"' value="{if isset($smarty.post.company)}{$smarty.post.company}{/if}" />
						    <sup>*</sup>
						</p>
                        <p class="required text" id="_dni"  {if $smarty.post.select_type == 1 || !isset($smarty.post.select_type)}style="display: none;"{/if}>
						...
						<p class="required text">
							...
							<input type="text" class="text" name="address1" id="address1" placeholder='Улица, дом, квартира, офис' value="{if isset($smarty.post.address1)}{$smarty.post.address1}{/if}">
							<sup>*</sup>
						</p>
						...
						{if $field_name eq "Customer:phone"}
...
{/literal}	
						...
						<p style="margin-left:50px;" id="h">Укажите домашний или мобильный телефон <sup style="color:red;">*</sup></p>
					    <p style="margin-left:50px;" id="j">Укажите рабочий или мобильный телефон <sup style="color:red;">*</sup></p>

						<p class="text" id="home" {if $smarty.post.select_type == 2 || !isset($smarty.post.select_type)}style="display: none;"{/if}>
							<label for="phone">{l s='Домашний телефон'}</label>
							<input type="text" class="text" name="phone" id="phone" placeholder='Например +7 (3452) 75-28-03' value="{if isset($smarty.post.phone)}{$smarty.post.phone}{/if}">
						
						</p>
						{elseif $field_name eq "Customer:job_mobile"}
		
						<p class="text" id="job" {if $smarty.post.select_type == 1 || !isset($smarty.post.select_type)}style="display: none;"{/if}>
			<label for="phone_mobile">{l s='Рабочий телефон'}</label>
			<input type="text" class="text" name="job_mobile" id="job_mobile" placeholder='Например +7 (3452) 75-28-03' value="{if isset($smarty.post.job_mobile)}{$smarty.post.job_mobile}{/if}" />
		...
		{/if}
					{/foreach}
		<p class="text" id="_company_"  {* {if $smarty.post.select_type == 1 || !isset($smarty.post.select_type)}style="display: none;"{/if} *}>
			<label for="phone_mobile">{l s='Mobile phone'}</label>
			<input type="text" class="text" name="phone_mobile" id="phone_mobile" placeholder='Например +7 (922) 000-00-00' value="{if isset($smarty.post.phone_mobile)}{$smarty.post.phone_mobile}{/if}" />
					...
					<p class="text">
			<label for="img"></label>
			<img src="/kcaptcha/index.php?rnd={$captcha}" border="0" alt="" style="margin-left: 191px;">
		</p>
					<p class="required text">
			<label for="capcha_code">{l s='Введите код с картинки'}</label>
			<input type="text" class="text" name="capcha_code" id="capcha_code" value="" />
	        <sup>*</sup>
		</p>
...
<form action="{$link->getPageLink('authentication.php', true)}" method="post" id="account-creation_form" class="std">
	...
	<fieldset class="account_creation">
		...
		<p class="required password">
			<label for="repasswd">{l s='Повторите пароль'}</label>
...
{/literal}	
		 <p class="radio required" style="margin-left: 16px; margin-bottom: -16px;">
                <input type="radio" name="select_type" id="select_type_1" value="1" {if $smarty.post.select_type == 1}checked="checked"{/if} />
                <label for="select_type_1" class="top">Физическое лицо</label>
                <input type="radio" name="select_type" id="select_type_2" value="2" {if $smarty.post.select_type == 2}checked="checked"{/if} />
                <label for="select_type_2" class="top">Юридическое лицо</label>
         </p>
		...
		{foreach from=$dlv_all_fields item=field_name}
			...
			{elseif $field_name eq "firstname"}
				{*<p class="required text">
					<label for="firstname">{l s='First name'}</label>*}
					<input type="hidden" class="text" id="firstname" name="firstname" value="{if isset($smarty.post.firstname)}{$smarty.post.firstname}{/if}" />
					{*<sup>*</sup>*}
				</p>
			{elseif $field_name eq "lastname"}
				{*<p class="required text">
					<label for="lastname">{l s='Last name'}</label>*}
					<input type="hidden" class="text" id="lastname" name="lastname" value="{if isset($smarty.post.lastname)}{$smarty.post.lastname}{/if}" />
					{*<sup>*</sup>*}
				</p>
			{elseif $field_name eq "address1"}
				<p class="required text">
					<label for="address1">{l s='Address'}</label>
					<input type="text" class="text" name="address1" id="address1" placeholder="{l s='Street address, P.O. box, company name, c/o'}" value="{if isset($smarty.post.address1)}{$smarty.post.address1}{/if}" />
					<sup id="addr1">*</sup>
			...
			{elseif $field_name eq "address2"}
				<p class="text">
					...
					<input type="text" class="text" name="address2" id="address2" placeholder="{l s='Apartment, suite, unit, building, floor, etc.'}" value="{if isset($smarty.post.address2)}{$smarty.post.address2}{/if}" />
		...
		<p class="required text" id="address_alias">
			<label for="alias">{l s='Название адреса'} </label>
			<input type="text" class="text" name="alias" id="alias" placeholder='Например: рабочий, домашний' value="{if isset($smarty.post.alias)}{$smarty.post.alias}{/if}" />
			<sup>*</sup>
		</p>
		
		
		<p style="margin-left:50px;" id="h">Укажите домашний, рабочий или мобильный телефон <sup style="color:red;">*</sup></p>
		<p style="margin-left:50px;" id="j">Укажите рабочий или мобильный телефон <sup style="color:red;">*</sup></p>
		<p class="text" id="home" {if $smarty.post.select_type == 2 || !isset($smarty.post.select_type)}style="display: none;"{/if}>
