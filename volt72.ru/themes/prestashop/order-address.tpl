		...
		{ldelim}
			...
			'formated_fields_values':{ldelim}
					{foreach from=$type.formated_fields_values key=pattern_name item=field_name name=inv_loop}
					  ...
					  {if $pattern_name != 'Customer:phone' && $pattern_name != 'Customer:job_mobile' && $pattern_name != 'Customer:phone_mobile'}
...
{if !$opc}<h1 style="width: 100%; font-size: 26px; font-weight: bold; color: gray; font-family: Calibri; clear: both; text-transform: uppenspace; margin-bottom: 8px;">{l s='Addresses'}</h1>{else}<h2>1. {l s='Addresses'}</h2>{/if}
{if !$opc}
...
{/if}
	<div class="addresses">
		<p class="address_delivery select">
			...
			<label for="id_phone_delivery" style="padding-left: 10px;">{l s='Выберите телефон:'}</label>
			<select name="id_phone_delivery" id="id_phone_delivery" class="address_select">
{foreach from=$addresses key=k item=address name=address}
{if $smarty.foreach.address.index == 0}
			{if $address.phone <> 0}<option value="1">{$address.phone}</option>{/if}
			{if $address.job_mobile <> 0}<option value="2">{$address.job_mobile}</option>{/if}
			{if $address.phone_mobile <> 0}<option value="3">{$address.phone_mobile}</option>{/if}
{/if}
{/foreach}			
			</select>
		</p>
